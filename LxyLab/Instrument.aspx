<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Instrument.aspx.cs" Inherits="LxyLab.Instrument1" %>
    ...
    <script type="text/javascript">
        $(function () {
            $("#userOrder-dg").datagrid({
                url: 'GetInstrumentList.ashx', pagination: true,
                columns: [[
                           { field: 'InstrumentID', title: '编号', checkbox: true },
                           { field: 'InstrumentName', title: '仪器名称' },
                           { field: 'InstrumentAmount', title: '仪器数量' },//InstrumentPer
                           { field: 'InstrumentPer', title: '共用人数' },
                           {
                               field: '_LabTypeID', title: '操作', align: 'center', formatter: function (value, rowData, rowIndex) {
                                   ...
                                   //+ '" >详细</a>'
                                   '<a herf="javascript:void(0)" class="easyui-linkbutton edit-dl-btn"  data-options="iconCls:\'icon-page_edit\' ,plain:true" data-id="' + rowData.InstrumentID
                                   + '" >修改</a>' +
                                   '<a herf="javascript:void(0)" class="easyui-linkbutton delete-dl-btn"  data-options="iconCls:\'icon-delete\',plain:true" data-id="' + rowData.InstrumentID
                                   + '" >删除</a>'
                ...
                ]],  toolbar: [
                   {
                       ...
                       iconCls: "icon-add",
                       handler: function () {
                           sr.dialog.form("EditInstrument.aspx",
                ...
                onLoadSuccess: function (data) {
                    ...
                    //    var $this = $(this);
                    //    sr.dialog.dialog({ title: " 详细信息", modal: true, width: 800, href: "OrderDetail.aspx?id=" + $this.attr("data-id") });
                    ...
                    */
                    $(".delete-dl-btn").linkbutton().on("click", function () {
                        ...
                        $.messager.confirm("请确认", "您确定要删除？", function (b) {
                            if (b) {
                                $.post("DeleteInstrument.ashx", { id: parseInt($this.attr("data-id")) }, function (data) {
